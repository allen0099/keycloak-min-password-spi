...
-   **Keycloak 版本**：目前已在 Keycloak 24.0.1 與 26.4.5 版本測試通過，理論上支援 Keycloak 24 到 26 的所有版本。
...
### 驗證說明
-   **UI 驗證**：Keycloak Admin UI 會驗證格式。如果您輸入無效的格式（例如：`abc`、`10:years`），UI 會顯示錯誤並阻止您儲存政策。
-   **負數**：不允許使用負數，UI 也會阻止儲存。
